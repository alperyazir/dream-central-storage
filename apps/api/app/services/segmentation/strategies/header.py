...
ROMAN_NUMERAL_PATTERN = re.compile(r"^(M{0,3})(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})$", re.IGNORECASE)
...
class HeaderBasedStrategy(SegmentationStrategy):
    ...
    def _build_patterns(self) -> list[tuple[Pattern, float]]:
        ...
        patterns.append((re.compile(rf"^({keywords})\s*(\d+)\s*[:\-–—]?\s*(.*)$", re.IGNORECASE | re.MULTILINE), 1.0))

        # Pattern 2: "Unit One", "Chapter Two" (with word numbers)
        number_words = "|".join(NUMBER_WORDS.keys())
        patterns.append(
            (re.compile(rf"^({keywords})\s+({number_words})\s*[:\-–—]?\s*(.*)$", re.IGNORECASE | re.MULTILINE), 0.95)
        )

        # Pattern 3: Roman numerals "I.", "II.", "III."
        patterns.append((re.compile(r"^([IVXLCDM]+)\.\s+(.+)$", re.IGNORECASE | re.MULTILINE), 0.8))

        # Pattern 4: Simple numbered "1.", "2.", "3." at line start
        patterns.append((re.compile(r"^(\d+)\.\s+([A-Z][a-zA-Z\s]{3,50})$", re.MULTILINE), 0.6))
