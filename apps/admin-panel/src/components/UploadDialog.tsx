...
import { uploadBookArchive, uploadNewBookArchive, uploadAppArchive } from 'lib/uploads'
...
export function UploadDialog({ open, onClose, books, token, tokenType, onSuccess }: UploadDialogProps) {
    ...
    try {
      if (mode === 'book') {
        if (bookUploadFlow === 'update' && selectedBookId) {
          await uploadBookArchive(Number(selectedBookId), selectedFile, token, tokenType, undefined, { override })
        } else {
          await uploadNewBookArchive(selectedFile, token, tokenType, undefined, { override })
        }
      } else {
        if (!selectedPlatform) return
        await uploadAppArchive(selectedPlatform, selectedFile, token, tokenType, undefined, { override })
      }
      setFeedback({ type: 'success', message: 'Upload completed successfully!' })
      onSuccess()
    } catch (error) {
      if (error instanceof ApiError && error.status === 409) {
        setOverrideContext({
          mode,
          bookId: bookUploadFlow === 'update' ? Number(selectedBookId) : undefined,
  ...
  const canSubmit =
    selectedFile &&
    !isSubmitting &&
    (mode === 'app' ? !!selectedPlatform : bookUploadFlow === 'new' || !!selectedBookId)
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {overrideContext ? (
          <div className="space-y-4">
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setOverrideContext(null)}>Cancel</Button>
              <Button variant="destructive" onClick={handleOverride}>Override</Button>
            </DialogFooter>
          </div>
        ) : (
          <>
            <Tabs value={mode} onValueChange={(v) => setMode(v as UploadMode)}>
              <TabsList className="w-full">
                ...
                <TabsTrigger value="app" className="flex-1">Application</TabsTrigger>
              </TabsList>

              <TabsContent value="book" className="space-y-4">
                <div className="space-y-2">
                  <Label>Upload Type</Label>
                  <Select value={bookUploadFlow} onValueChange={(v) => setBookUploadFlow(v as BookUploadFlow)}>
