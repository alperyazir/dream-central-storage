...
export function AIDataDialog({
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [meta, mods, vocab, acts] = await Promise.all([
        ...
        getAIModules(bookId, token, tokenType).catch(() => ({
          modules: [] as ModuleSummary[],
        })),
        getAIVocabulary(bookId, token, tokenType).catch(() => ({
          words: [] as VocabularyWord[],
        })),
        listAIContent(bookId, token, tokenType).catch(
          () => [] as ManifestRead[]
        ),
      ]);
      setMetadata(meta);
  ...
  const handleDeleteActivity = async () => {
    ...
    try {
      ...
      setActivities((p) =>
        p.filter((a) => a.content_id !== deleteTarget.content_id)
      );
              ...
              {!modules.length ? (
                ...
                <div className="space-y-1">
                  {modules.map((m) => (
                    <div key={m.module_id}>
                      <button
                        className="flex w-full items-center justify-between rounded-md p-2.5 text-sm hover:bg-muted transition-colors text-left"
                          ...
                          <div className="ml-4 space-y-2 border-l pl-4 pb-2 text-sm">
                            ...
                            {moduleDetails[m.module_id].text && (
                              <pre className="max-h-[150px] overflow-auto rounded-md bg-muted p-2 text-xs whitespace-pre-wrap">
                                {moduleDetails[m.module_id].text.slice(0, 500)}
                              </pre>
                            )}
                          </div>
                        )}
                ...
                <div className="space-y-1">
                  {activities.map((a) => (
                    <div key={a.content_id} className="rounded-md border">
                      <button
                        className="flex w-full items-center gap-2 p-2.5 text-sm hover:bg-muted transition-colors text-left"
                        onClick={() => toggleActivity(a.content_id)}
                      >
                      ...
                      {expandedActivity === a.content_id && (
                        <div className="border-t p-3 space-y-2">
                          <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                            ...
                            {a.created_by && (
                              <span>By: {a.created_by.slice(0, 12)}</span>
                            )}
                            {a.created_at && (
