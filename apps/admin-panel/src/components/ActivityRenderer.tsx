...
export function ActivityRenderer({
  ...
  const AudioBtn = ({
    ...
    <Button
      ...
      onClick={(e) => {
        e.stopPropagation();
        playingKey === audioKey ? stopAudio() : onPlay();
      }}
    >
      {playingKey === audioKey ? (
  ...
  const storageAudio = (data.audio_files ??
    data.storage_audio ??
    []) as StorageAudioFile[];
        ...
        <div className="space-y-1.5">
          {items.map((item, i) => {
            ...
            return (
              ...
              >
                <span className="text-muted-foreground min-w-[20px] pt-0.5">
                  {i + 1}.
                </span>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {item.options.map((opt, j) => {
                        const isCorrect =
                          ...
                          item.correct_answer === j ||
                          item.answer === opt ||
                          Number(item.answer) === j;
                        return (
                          <Badge
                            key={j}
                ...
                {hasBase64 && (
                  <AudioBtn
                    audioKey={audioKey}
                    onPlay={() =>
                      playBase64(audioKey, item.audio_data!.audio_base64)
                    }
                  />
                )}
              </div>
      ...
      {storageAudio.length > 0 && (
        <div className="space-y-1">
          <div className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
            <Volume2 className="h-3.5 w-3.5" /> Audio Files (
            {storageAudio.length})
          </div>
          {storageAudio.map((af, i) => {
            ...
            return (
              ...
              >
                <AudioBtn
                  audioKey={audioKey}
                  onPlay={() => playStorageAudio(audioKey, af.filename)}
                />
