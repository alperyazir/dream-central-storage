...
const DashboardPage = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([bks, tpls, bnds, ...appTrees]) => {
        ...
        setAppStorageSize(
          (appTrees as (StorageNode | undefined)[]).reduce(
            (acc, t) => acc + sumStorageTree(t),
            0
          )
        );
      })
  ...
  const publishers = useMemo(
    () => new Set(books.map((b) => b.publisher)).size,
    [books]
  );
  const appBuildCount = templates.length + bundles.length;

  const totalBookSize = useMemo(
    () => books.reduce((a, b) => a + (b.total_size || 0), 0),
    [books]
  );
  const totalTemplateSize = useMemo(
    () => templates.reduce((a, b) => a + b.file_size, 0),
    [templates]
  );
  const totalBundleSize = useMemo(
    () => bundles.reduce((a, b) => a + b.file_size, 0),
    [bundles]
  );
  const totalStorage =
    totalBookSize + totalTemplateSize + totalBundleSize + appStorageSize;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Review stored content at a glance.
          </p>
        </div>
