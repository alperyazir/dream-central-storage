...
const TrashPage = () => {
  ...
  const filtered = useMemo(() => {
    const d = typeFilter
      ? entries.filter((e) => e.item_type === typeFilter)
      : entries;
    return [...d].sort((a, b) =>
  ...
  const doRestore = async () => {
    ...
    setRestoring(true);
    try {
      await restoreTrashEntry(restoreTarget.key, token, tt);
      setNotification({ message: 'Restored!', severity: 'success' });
      setRestoreTarget(null);
      setRefresh((v) => v + 1);
    } catch {
      setNotification({ message: 'Restore failed', severity: 'error' });
    } finally {
      setRestoring(false);
    }
  };
  ...
  const doDelete = async () => {
    ...
    setDeleting(true);
    try {
      await deleteTrashEntry(deleteTarget.key, token, tt, undefined, {
        force: true,
      });
      setNotification({ message: 'Permanently deleted', severity: 'success' });
              ...
              {!filtered.length ? (
                ...
                filtered.map((e) => (
                  <TableRow key={e.key}>
                    ...
                    <TableCell>
                      <Badge variant="outline">{typeLabel(e.item_type)}</Badge>
                    </TableCell>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Restore &quot;{restoreTarget ? getEntryLabel(restoreTarget) : ''}
              &quot;?
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Permanently delete &quot;
              {deleteTarget ? getEntryLabel(deleteTarget) : ''}&quot;? This
              cannot be undone.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
